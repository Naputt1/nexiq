...
2.  **Universal Backend**: A shared `ProjectManager` and server that provides data to both the MCP server and the external `nexiq-ui` application.
...
- **`nexiq-ui` (External)**: The Electron desktop app, now a consumer of the shared backend via WebSocket (hosted in a separate repository).
...
- [x] Connect external UI to the shared backend via WebSocket.
