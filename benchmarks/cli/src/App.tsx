...
const App = () => {
  ...
  const [finishedResultPath, setFinishedResultPath] = useState<string | null>(
    null,
  );
  ...
  const startBenchmarks = async () => {
    ...
    try {
      ...
      if (openViewerWhenDone) {
        setTimeout(() => {
          open("http://localhost:5173").catch((e) =>
  ...
  const renderTree = () => {
    const projects = Array.from(
      new Set(Object.values(tasks).map((t) => t.projectName)),
    );
    ...
    return (
      <Box flexDirection="column">
        {projects.map((p) => {
          ...
          const successCount = projectTasks.filter(
            (t) => t.status === "success",
          ).length;
          const runningCount = projectTasks.filter(
            (t) => t.status === "running",
          ).length;
          ...
          return (
            <Box key={p} flexDirection="column" marginLeft={2}>
              <Text bold color="cyan">
                {p}{" "}
                <Text color="gray">
                  ({successCount}/{projectTasks.length} done, {runningCount}{" "}
                  active)
                </Text>
              </Text>
                ...
                return (
                  <Box key={taskId} marginLeft={2}>
                    ...
                    {t.status === "running" && (
                      <Text color="blue">
                        It: {t.iteration || 0}{" "}
                        {t.lastTool ? `| Tool: ${t.lastTool}` : ""}
                      </Text>
                    )}
                    {t.status === "success" && (
                      <Text color="green">({t.tokens} tox)</Text>
                    )}
                  </Box>
                );
              })}
      ...
      {step === "running" && (
        <Box flexDirection="column">
          <Box>
            ...
            <Text bold>
              {" "}
              Running Benchmarks ({completedCount}/{totalScenarios})
            </Text>
          </Box>
          <Box
            borderStyle="round"
        ...
        <Box flexDirection="column">
          ...
          >
            ...
            <Text>
              ...
              {completedCount > 0
                ? (
                    (Object.values(tasks).filter((r) => r.status === "success")
                      .length /
                      completedCount) *
                    100
                  ).toFixed(1)
          ...
          <Box flexDirection="column">
            ...
            {openViewerWhenDone && (
              <Box marginTop={1}>
                <Text color="blue">
                  Opening viewer at http://localhost:5173...
                </Text>
              </Box>
            )}
          </Box>
