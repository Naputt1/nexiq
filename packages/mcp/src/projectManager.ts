...
export class ProjectManager {
  ...
  async openProject(projectPath: string, subProject?: string): Promise<ProjectInfo> {
    const key = subProject ? `${projectPath}:${subProject}` : projectPath;
    if (this.projects.has(key)) {
      return this.projects.get(key)!;
    }

    const analysisPath = subProject ? path.resolve(projectPath, subProject) : projectPath;
    const cacheDir = path.join(analysisPath, ".react-map", "cache");
    if (!fs.existsSync(cacheDir)) {
      fs.mkdirSync(cacheDir, { recursive: true });
    }

    const cacheFile = path.join(cacheDir, `${path.basename(analysisPath)}.json`);
    ...
    for (const name of extensionNames) {
      try {
        ...
        const extensionSlug = name.replace("@react-map/", "").replace("-extension", "");
        const extPath = path.join(monorepoRoot, "extensions", extensionSlug, "dist", "index.js");
        ...
        const extension = Object.values(loaded).find(
          (val: any) => val && typeof val === "object" && "id" in val
        ) as Extension;
        ...
        console.error(`Failed to load extension ${name}:`, e.message);
      }
    }
      ...
      const subscription = await watcher.subscribe(
        ...
        (err, events) => {
          ...
          if (hasRelevantChange) {
            console.error(`Changes detected in ${analysisPath}, re-analyzing...`);
            try {
              projectInfo.graph = analyzeProject(analysisPath, cacheFile, ignorePatterns);
              fs.writeFileSync(cacheFile, JSON.stringify(projectInfo.graph, null, 2));
              console.error(`Project ${analysisPath} re-analyzed successfully.`);
            } catch (reAnalyzeError) {
              console.error(`Re-analysis failed for ${analysisPath}:`, reAnalyzeError);
            }
          }
        },
        {
          ignore: [
            ...
            ".next",
            ".vite",
            ...(ignorePatterns || []).map((p) => p.replace(/^\*\*\/|\/\*\*$/g, "")),
          ],
        }
      );
      projectInfo.subscription = subscription;
    } catch (watcherError) {
      console.error(`Failed to start watcher for ${analysisPath}:`, watcherError);
    }
  ...
  getProject(projectPath: string, subProject?: string): ProjectInfo | undefined {
    const key = subProject ? `${projectPath}:${subProject}` : projectPath;
    return this.projects.get(key);
  }
