...
export const componentTask: GraphViewTask = {
  ...
  run: (graphData: JsonData, result: GraphViewResult): GraphViewResult => {
    ...
    const { deletedObjects = {} } = graphData.diff || {};
    ...
    const getLeafNames = (name: VariableName): string[] => {
      if (name.type === "identifier") return [name.name];
      if (name.type === "rest") return getLeafNames(name.argument);
      if (name.type === "array") {
        return name.elements.flatMap((e) => (e ? getLeafNames(e.value) : []));
      }
      if (name.type === "object") {
        return name.properties.flatMap((p) => getLeafNames(p.value));
      }
      return [];
    };
      ...
      if ("var" in variable && variable.var) {
        for (const v of Object.values(
          ...
          const nodeBase: GraphNodeData = {
            ...
            raw: v,
            displayName: getDisplayName(v.name),
          };
          ...
          if (v.kind == "state") {
            ...
            const addDestructiveVariable = (
              ...
              const leaves = getLeafNames(v);
              const resolvedName =
                leaves.length === 1 && leaves[0] ? leaves[0] : parentName;
            ...
            if (v.type === "jsx") {
              ...
              const targetV =
                findVariableById(tagId) ||
                graphData.files[filePath]?.var[tagId];
              if (targetV) {
                edges.push({
                  id: `${v.id}-${targetV.id}`,
                  source: v.id,
